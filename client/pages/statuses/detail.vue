...
export default {
  ...
  apollo: {
    status: {
      query: graphql.queries.getStatus,
      variables() {
        return {
      ...
      result(result) {
        ...
        this.parentStatus = status.parent;
        this.emailTemplate = status.emailTemplate;
        this.smsTemplate = status.smsTemplate;
        if (!status.parent) {
  ...
  methods: {
    async handleSubmit() {
      ...
      await this.$apollo.mutate({
        ...
        variables: { id: this.$route.params.id, input: payload }
      });
      this.saving = false;
